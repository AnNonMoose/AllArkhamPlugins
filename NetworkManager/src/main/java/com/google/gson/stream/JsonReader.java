 ...
 *       #beginArray} to consume the array's opening bracket. Then create a while loop that accumulates
 * values, terminating when {@link #hasNext} is false. Finally, read the array's closing bracket by
 * calling {@link
 *       #endArray}.
 * <li>Within <strong>object handling</strong> methods, first call {@link
 *       #beginObject} to consume the object's opening brace. Then create a while loop that assigns values
 * to local variables based on their name. This loop should terminate when {@link #hasNext} is
 * false. Finally, read the object's closing brace by calling {@link #endObject}.
 * </ul>
 * <p>
 ...
 * consumed using either {@link #nextNull()} or {@link
 * #skipValue()}.
 *
 * <h3>Example</h3>
 ...
 * For example, both elements of the JSON array {@code
 * [1, "1"]} may be read using either {@link #nextInt} or {@link #nextString}. This behavior is
 * intended to prevent lossy numeric conversions: double is JavaScript's only numeric type and very
 * large values like {@code
 * 9007199254740993} cannot be represented exactly on that platform. To minimize precision loss,
 * extremely large values should be written and read as strings in JSON.
 *
 ...
 * encountered. This class permits the non-execute prefix when
 * {@link #setLenient(boolean) lenient parsing} is enabled.
 *
 * <p>
...
public class JsonReader implements Closeable {
      ...
      /**
       * Use a manual buffer to easily read and unread upcoming characters, and also so we can
       * create strings without an intermediate StringBuilder. We decode literals directly out of
       * this buffer, so it must be at least as long as the longest token that can be reported as a
       * number.
       */
      ...
      /**
       * The number of characters in a peeked number literal. Increment 'pos' by this after reading
       * a number.
       */
       ...
       * href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>. Setting the parser to lenient
       * causes it to ignore the following syntax errors:
       *
       * <ul>
       ...
       *
       * @throws IllegalStateException if the next token is not a string or if this reader is
       * closed.
       */
      ...
      /**
       * Returns the {@link com.google.gson.stream.JsonToken#BOOLEAN boolean} value of the next
       * token, consuming it.
       *
       * @throws IllegalStateException if the next token is not a boolean or if this reader is
       * closed.
       */
       ...
       * consuming it. If the next token is a string, this method will attempt to parse it as a
       * double using {@link Double#parseDouble(String)}.
       *
       ...
       * consuming it. If the next token is a string, this method will attempt to parse it as a
       * long. If the next token's numeric value cannot be exactly represented by a Java
       * {@code long}, this method throws.
       *
       * @throws IllegalStateException if the next token is not a literal value.
       ...
       * consuming it. If the next token is a string, this method will attempt to parse it as an
       ...
       * skipped. This method is intended for use when the JSON token stream contains unrecognized
       * or unhandled values.
       */
      ...
      /**
       * Returns the next character in the stream that is neither whitespace nor a part of a
       * comment. When this returns, the returned character is always at {@code buffer[pos-1]}; this
       * means the caller can always push back the returned character by decrementing {@code pos}.
       */
