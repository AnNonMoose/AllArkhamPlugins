 ...
 * adapters will write enums in lowercase, despite the fact that they're defined in
 ...
 * should expect {@code
 * create()} to be called on them for many types and should return null for most of those types. In
 * the above example the factory returns null for calls to {@code create()} where {@code type} is
 * not an enum.
 *
 * <p>
 ...
 * type adapter's {@code
 * read()} and {@code write()} methods can be very fast. In this example the mapping from lowercase
 * name to enum value is computed eagerly.
 *
 * <p>
 ...
 * }</pre> If multiple factories support the same type, the factory registered earlier takes
 * precedence.
 *
 * <h3>Example: composing other type adapters</h3>
 * In this example we implement a factory for Guava's {@code Multiset} collection type. The factory
 * can be used to create type adapters for multisets of any element type: the type adapter for {@code
 ...
 *     }
 *   }
 * }</pre> Delegating from one type adapter to another is extremely powerful; it's the foundation of
 * how Gson converts Java objects and collections. Whenever possible your factory should retrieve
 * its delegate type adapter in the {@code create()} method; this ensures potentially-expensive type
...
public interface TypeAdapterFactory {
      ...
      /**
       * Returns a type adapter for {@code type}, or null if this factory doesn't support
       * {@code type}.
       */
