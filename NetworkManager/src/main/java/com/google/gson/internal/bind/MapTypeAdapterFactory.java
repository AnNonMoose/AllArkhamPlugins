 ...
 *     "(8,8)": "b"
 *   }
 * }</pre> But GSON is unable to deserialize this value because the JSON string name is just the
 ...
 *     .create();
 * }</pre> This will change the structure of the JSON emitted by the code above. Now we get an
 * array. In this case the arrays elements are map entries:
 * <pre>   {@code
 *   [
 *     [
 *       {
 ...
 *     ]
 *   ]
 * }</pre> This format will serialize and deserialize just fine as long as this adapter is
 * registered.
 */
