...
public class Unzip {
    ...
    public void extractAll(final UnzipParameters unzipParameters,
            final String outPath, final ProgressMonitor progressMonitor,
        ...
        if (runInThread) {
            ...
            initExtractAll(fileHeaders, unzipParameters, progressMonitor,
                    outPath);
        }

    }

    private void initExtractAll(ArrayList fileHeaders,
            UnzipParameters unzipParameters, ProgressMonitor progressMonitor,
    ...
    public void extractFile(final FileHeader fileHeader, final String outPath,
        ...
        if (runInThread) {
            Thread thread = new Thread(InternalZipConstants.THREAD_NAME) {
                public void run() {
                    try {
                        initExtractFile(fileHeader, outPath, unzipParameters,
                                newFileName, progressMonitor);
                        progressMonitor.endProgressMonitorSuccess();
                    } catch (ZipException e) {
                    }
                }
    ...
    private void initExtractFile(FileHeader fileHeader, String outPath,
        ...
        try {
            ...
            if (fileHeader.isDirectory()) {
                ...
                try {
                    unzipEngine.unzipFile(progressMonitor, outPath,
                            newFileName, unzipParameters);
                } catch (Exception e) {
                    progressMonitor.endProgressMonitorError(e);
                    throw new ZipException(e);
                }
    ...
    private void checkOutputDirectoryStructure(FileHeader fileHeader,
            String outPath, String newFileName) throws ZipException {
        if (fileHeader == null
                || !Zip4jUtil.isStringNotNullAndNotEmpty(outPath)) {
            throw new ZipException(
                    "Cannot check output directory structure...one of the parameters was null");
        }
    ...
    private long calculateTotalWork(ArrayList fileHeaders) throws ZipException {
        ...
        if (fileHeaders == null) {
            throw new ZipException(
                    "fileHeaders is null, cannot calculate total work");
        }
        ...
        for (int i = 0; i < fileHeaders.size(); i++) {
            ...
            if (fileHeader.getZip64ExtendedInfo() != null
                ...
                totalWork += fileHeader.getZip64ExtendedInfo()
                        .getCompressedSize();
            } else {
