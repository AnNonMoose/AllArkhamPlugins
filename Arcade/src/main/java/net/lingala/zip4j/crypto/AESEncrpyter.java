...
public class AESEncrpyter implements IEncrypter {
    ...
    public AESEncrpyter(char[] password, int keyStrength) throws ZipException {
        if (password == null || password.length == 0) {
            throw new ZipException(
                    "input password is empty or null in AES encrypter constructor");
        }
        if (keyStrength != Zip4jConstants.AES_STRENGTH_128
                && keyStrength != Zip4jConstants.AES_STRENGTH_256) {
            throw new ZipException(
                    "Invalid key strength in AES encrypter constructor");
        }
    ...
    private void init() throws ZipException {
        ...
        if (keyBytes == null
                || keyBytes.length != (KEY_LENGTH + MAC_LENGTH + PASSWORD_VERIFIER_LENGTH)) {
        ...
        System.arraycopy(keyBytes, KEY_LENGTH + MAC_LENGTH,
                derivedPasswordVerifier, 0, PASSWORD_VERIFIER_LENGTH);
    ...
    private byte[] deriveKey(byte[] salt, char[] password) throws ZipException {
        try {
            ...
            byte[] derivedKey = e.deriveKey(password, KEY_LENGTH + MAC_LENGTH
    ...
    public int encryptData(byte[] buff) throws ZipException {
        ...
        if (buff == null) {
            throw new ZipException(
                    "input bytes are null, cannot perform AES encrpytion");
        }
    ...
    public int encryptData(byte[] buff, int start, int len) throws ZipException {
        ...
        if (finished) {
            ...
            throw new ZipException(
                    "AES Encrypter is in finished state (A non 16 byte block has already been passed to encrypter)");
        }
        ...
        for (int j = start; j < (start + len); j += InternalZipConstants.AES_BLOCK_SIZE) {
            loopCount = (j + InternalZipConstants.AES_BLOCK_SIZE <= (start + len)) ? InternalZipConstants.AES_BLOCK_SIZE
                    : ((start + len) - j);
