...
public class StandardDecrypter implements IDecrypter {
    ...
    public StandardDecrypter(FileHeader fileHeader, byte[] headerBytes)
            throws ZipException {
        if (fileHeader == null) {
            throw new ZipException(
                    "one of more of the input parameters were null in StandardDecryptor");
        }
    ...
    public int decryptData(byte[] buff, int start, int len) throws ZipException {
        if (start < 0 || len < 0) {
            throw new ZipException(
                    "one of the input parameters were null in standard decrpyt data");
        }
    ...
    public void init(byte[] headerBytes) throws ZipException {
        ...
        if (fileHeader.getPassword() == null
                || fileHeader.getPassword().length <= 0) {
        ...
        try {
            ...
            for (int i = 0; i < InternalZipConstants.STD_DEC_HDR_SIZE; i++) {
                ...
                zipCryptoEngine.updateKeys((byte) (result ^ zipCryptoEngine
                        .decryptByte()));
                if (i + 1 != InternalZipConstants.STD_DEC_HDR_SIZE) {
