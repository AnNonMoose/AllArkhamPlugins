...
public class CipherOutputStream extends BaseOutputStream {
    ...
    public void putNextEntry(File file, ZipParameters zipParameters)
        ...
        try {
            ...
            if (!zipParameters.isSourceExternalStream()) {
                ...
                if (!Zip4jUtil.isStringNotNullAndNotEmpty(this.zipParameters
                        .getFileNameInZip())) {
            ...
            if (this.outputStream instanceof SplitOutputStream) {
                if (totalBytesWritten == 4) {
                    ...
                    fileHeader
                            .setOffsetLocalHeader(((SplitOutputStream) outputStream)
                                    .getFilePointer());
                }
            } else {
                if (totalBytesWritten == 4) {
    ...
    private void initZipModel(ZipModel zipModel) {
        ...
        if (this.outputStream instanceof SplitOutputStream) {
            if (((SplitOutputStream) outputStream).isSplitZipFile()) {
                ...
                this.zipModel.setSplitLength(((SplitOutputStream) outputStream)
                        .getSplitLength());
            }
        }

        this.zipModel.getEndCentralDirRecord().setSignature(
        ...
        if (zipParameters.isEncryptFiles()
            ...
            if (pendingBufferLength != 0) {
                if (len >= (InternalZipConstants.AES_BLOCK_SIZE - pendingBufferLength)) {
                    ...
                    System.arraycopy(b, off, pendingBuffer,
            ...
            if (len != 0 && len % 16 != 0) {
                System.arraycopy(b, (len + off) - (len % 16), pendingBuffer, 0,
                        len % 16);
    ...
    private void createFileHeader() throws ZipException {
        ...
        if (zipParameters.isEncryptFiles()
            ...
            fileHeader
                    .setAesExtraDataRecord(generateAESExtraDataRecord(zipParameters));
        } else {
            fileHeader.setCompressionMethod(zipParameters
        ...
        if (zipParameters.isSourceExternalStream()) {
            fileHeader.setLastModFileTime((int) Zip4jUtil.javaToDosTime(System
                    .currentTimeMillis()));
            if (!Zip4jUtil.isStringNotNullAndNotEmpty(zipParameters
                    .getFileNameInZip())) {
                throw new ZipException("fileNameInZip is null or empty");
            }
            fileName = zipParameters.getFileNameInZip();
        } else {
            fileHeader.setLastModFileTime((int) Zip4jUtil
                    .javaToDosTime((Zip4jUtil.getLastModifiedFileTime(
        ...
        if (Zip4jUtil.isStringNotNullAndNotEmpty(zipModel.getFileNameCharset())) {
            fileHeader.setFileNameLength(Zip4jUtil.getEncodedStringLength(
                    fileName, zipModel.getFileNameCharset()));
        } else {
            fileHeader.setFileNameLength(Zip4jUtil
                    .getEncodedStringLength(fileName));
        }
                ...
                if (zipParameters.getCompressionMethod() == Zip4jConstants.COMP_STORE) {
                    if (zipParameters.getEncryptionMethod() == Zip4jConstants.ENC_METHOD_STANDARD) {
                        ...
                        switch (zipParameters.getAesKeyStrength()) {
                            ...
                            default:
                                throw new ZipException(
                                        "invalid aes key strength, cannot determine key sizes");
                        }
                        fileHeader.setCompressedSize(fileSize + saltLength
                                + InternalZipConstants.AES_AUTH_LENGTH + 2); // 2
                        // is
                        // password
                        // verifier
                    } else {
        ...
        shortByte[0] = Raw
                .bitArrayToByte(generateGeneralPurposeBitArray(
                                fileHeader.isEncrypted(),
                                zipParameters.getCompressionMethod()));
        boolean isFileNameCharsetSet = Zip4jUtil
                .isStringNotNullAndNotEmpty(zipModel.getFileNameCharset());
        if ((isFileNameCharsetSet && zipModel.getFileNameCharset()
                .equalsIgnoreCase(InternalZipConstants.CHARSET_UTF8))
                || (!isFileNameCharsetSet && Zip4jUtil.detectCharSet(
                        fileHeader.getFileName()).equals(
    ...
    private void createLocalFileHeader() throws ZipException {
        ...
        localFileHeader.setVersionNeededToExtract(fileHeader
        ...
        localFileHeader.setAesExtraDataRecord(fileHeader
                .getAesExtraDataRecord());
        localFileHeader.setCrc32(fileHeader.getCrc32());
        localFileHeader.setCompressedSize(fileHeader.getCompressedSize());
        localFileHeader.setGeneralPurposeFlag((byte[]) fileHeader
                .getGeneralPurposeFlag().clone());
    }
    ...
    private AESExtraDataRecord generateAESExtraDataRecord(
        ...
        if (parameters == null) {
            throw new ZipException(
                    "zip parameters are null, cannot generate AES Extra Data record");
        }
        ...
        if (parameters.getAesKeyStrength() == Zip4jConstants.AES_STRENGTH_128) {
            ...
            throw new ZipException(
                    "invalid AES key strength, cannot generate AES Extra data record");
        }
