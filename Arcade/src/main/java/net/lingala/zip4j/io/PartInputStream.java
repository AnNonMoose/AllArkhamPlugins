...
public class PartInputStream extends BaseInputStream {
    ...
    public PartInputStream(RandomAccessFile raf, long start, long len,
            ...
            if (decrypter != null && decrypter instanceof AESDecrypter) {
                ...
                if (readLen != InternalZipConstants.AES_AUTH_LENGTH) {
                    if (unzipEngine.getZipModel().isSplitArchive()) {
                        raf.close();
                        raf = unzipEngine.startNextSplitFile();
                        int newlyRead = raf.read(macBytes, readLen,
                                InternalZipConstants.AES_AUTH_LENGTH - readLen);
                        readLen += newlyRead;
                    } else {
                        throw new IOException(
                                "Error occured while reading stored AES authentication bytes");
                    }
                }

                ((AESDecrypter) unzipEngine.getDecrypter())
                        .setStoredMac(macBytes);
            }
        }
    }
